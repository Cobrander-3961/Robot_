...
-----------------
  1. Under Connectivity open \b SDMMC1 Configuration:
     - <b>DMA Settings</b>: setup DMA transfers for Rx\n
       \b Add - Select \b SDIO_RX: Stream=DMA2 Channel 5, Direction=Peripheral to Memory, Priority=Low
...
static uint32_t * ptr_wr_data = NULL;
...
//    }
    ...
    MCI.dlen  = cnt;
    MCI.dctrl = dctrl | (sz << 4) | SDMMC_DCTRL_DTEN;
    
    ptr_wr_data = (uint32_t *)((uint32_t)&data[0]);
  }
  else {
    ...
    SDMMC1->DLEN = cnt;
    SDMMC1->DCTRL= dctrl | (sz << 4) | SDMMC_DCTRL_DMAEN | SDMMC_DCTRL_DTEN;
  }
  ...
  if (sta & SDMMC_STA_CMDREND) {
    ...
    if (MCI.flags & MCI_DATA_XFER) {
      ...
      if (ptr_wr_data != NULL) {
        ...
        /* Fill FIFO */
        while (MCI.dlen != 0) {
          if ((SDMMC1->STA & SDMMC_STA_TXFIFOF) == 0) {
            MCI.dlen -= 4;
            SDMMC1->FIFO = *ptr_wr_data++;
          }
        }
        ptr_wr_data = NULL;
      }
